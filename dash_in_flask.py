...
app.layout = html.Div(children=[
	...
	dcc.RadioItems(
		id='dataset_radio',
		options=[{'label': k, 'value': k} for k in dataset],
		value='historical'
	),
	html.Hr(),
		...
		html.Div([
			...
			dcc.Dropdown(
			id='time_start_dropdown'
			),
			html.H4('Select end time:'),
			dcc.Dropdown(
			id='time_end_dropdown'
			)
			],
...
@app.callback(
	...
	[dash.dependencies.Input('dataset_radio', 'value')])
def set_cities_options(selected_dataset):
	return u'You have selected {}'.format(selected_dataset)

# callback: retrieve data for live or historical, set date dropdown options
@app.callback(
	dash.dependencies.Output('date_dropdown', 'options'),
	[dash.dependencies.Input('dataset_radio', 'value')])
def set_date_dropdown_options(selected_dataset):
	
	if selected_dataset == 'live':
		return [{'label': i, 'value': i} for i in ['None']]
	if selected_dataset == 'historical':
		# read the dates that are available
		# return as list
		return [{'label': i, 'value': i} for i in [1]]

# callback to retrieve the data and plot it
@app.callback(
	dash.dependencies.Output('indicator-graphic', 'figure'),
	[dash.dependencies.Input('date_dropdown', 'value'), dash.dependencies.Input('dataset_radio', 'value')])
def update_figure(selected_date, dataset):
	
	if dataset == 'live':
		df_dose, df_HV, df_pressure = retrieveLiveData(7200)  # retrieve the past 2 hrs
		# plot each
